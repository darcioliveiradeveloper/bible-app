...
export default function ProgressScreen({ onBack, onNavigateToChapterSelector }) {
  const progressData = [
    ...
    { title: 'Apocalipse', isHeader: false, percent: '0%', count: '0/22' },
  ];

  return (
   <View style={styles.container}>
      <Header title="Progresso de Leitura" />
      ...
      <ScrollView contentContainerStyle={styles.listContent}>
        {progressData.map((item, index) => (
          <TouchableOpacity 
            ...
            onPress={() => !item.isHeader && onNavigateToChapterSelector(item.title)}
          >
